#!/usr/bin/env python3
"""Fetch a fresh batch of Japanese verb-conjugation fill-in-the-blank questions
and reshape them for a quiz session.

POSTs to the quiz-service generate endpoint with ``lang=ja&slug=verb-conjugation``,
then:
  * drops the ``keyword`` field from every question, and
  * splits the payload into two JSON documents on stdout:
      - ``questions``: the user-facing exam (number, category, type, sentenceQuiz,
        translation, hintZh) — NO rightAnswer/keyword,
      - ``answers``:   a grading map keyed by question number, holding
        rightAnswer, sentence, category, type and keyword for the agent to check.

The exam question is ``sentenceQuiz`` (the 【】-blanked sentence); ``sentence`` is
the reconstructed source only kept in ``answers`` so the correct answer can be
shown after grading.
"""
...
def fetch_batch(base_url: str = BASE_URL) -> dict:
    url = f"{base_url}?lang=ja&slug=verb-conjugation"
